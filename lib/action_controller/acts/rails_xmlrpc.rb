    ...
    module RailsXmlrpc
      ...
      module InstanceMethods
        ...
        def xe_index
          ...
          render xml: result, content_type: 'text/xml'
        end
        ...
        def add_method_handlers
          ...
          self.class.xe_method_list.each do |method|
            puts "Adding XMLRPC method for #{method[:name].to_s}" unless Rails.env.test?
          ...
          self.class.instance_methods(false).each do |method|
            unless ['xe_index', 'xe_method_prefix', 'xe_method_list'].member?(method)
              puts "Adding XMLRPC method for #{method.to_s}" unless Rails.env.test?
